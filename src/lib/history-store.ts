...
/** Prefer site slug from runtime config when available (set by layout). */
function historyStorageKey(): string {
  if (typeof window !== "undefined") {
    const slug = (window as Window & { __SITE_CONFIG__?: { slug?: string } }).__SITE_CONFIG__
      ?.slug;
    if (slug) return `${slug}.history.v1`;
  }
  return "site.history.v1";
}

/** @deprecated Use historyStorageKey() — kept for callers that need a string snapshot. */
export const HISTORY_STORAGE_KEY = "site.history.v1";
const MAX_ITEMS = 48;
