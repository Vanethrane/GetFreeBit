...
export async function renderShareCardPng(payload: SharePayload): Promise<Blob> {
  ...
  gradient.addColorStop(0, siteConfig.colors.voice);
  gradient.addColorStop(1, siteConfig.colors.voiceDark);
  ...
  ctx.fillStyle = siteConfig.colors.voiceGlow;
  ctx.font = "600 36px Georgia, 'Times New Roman', serif";
  ctx.fillText(siteConfig.name, 96, 118);
